    ...
    with get_db() as db:
        ...
        def _mark_cancelled(reason: str = "任务已取消") -> None:
            ...
            task_manager.add_log(
                task_uuid,
                f"{log_prefix} [取消] {safe_reason}"
                if log_prefix
                else f"[取消] {safe_reason}",
            )
        ...
        try:
            ...
            if result.success:
                ...
                if auto_upload_tm:
                    _raise_if_cancelled(
                        "任务在 Team Manager 上传前收到取消请求，停止后处理"
                    )
                    try:
...
async def run_batch_parallel(
    ...
    try:
        ...
        if not task_manager.is_batch_cancelled(batch_id):
            add_batch_log(
                f"[完成] 批量任务完成！成功: {batch_tasks[batch_id]['success']}, 失败: {batch_tasks[batch_id]['failed']}"
            )
...
async def cancel_task(task_uuid: str):
    ...
    with get_db() as db:
        ...
        task_manager.add_log(
            task_uuid, "[取消] 任务取消请求已提交，等待后台线程有序退出"
        )
